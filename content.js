...
async function saveCacheEntry(username, locationData) {
  // Check if extension context is still valid
  if (!chrome.runtime?.id) {
  ...
  locationCache.set(username, locationData);
  ...
  return new Promise((resolve, reject) => {
    ...
    const handler = (event) => {
      ...
      if (event.data &&
        ...
        const locationAccurate = event.data.locationAccurate;
        ...
        if (!isRateLimited) {
          saveCacheEntry(screenName, { location: location || null, locationAccurate: locationAccurate ?? null });
        } else {
          console.log(`Not caching null for ${screenName} due to rate limit`);
        }

        resolve({ location: location || null, locationAccurate: locationAccurate ?? null });
      }
    };
...
async function getUserLocation(screenName) {
  ...
  if (locationCache.has(screenName)) {
    ...
    const location = typeof cached === 'string' ? cached : cached?.location;
    ...
    if (location !== null && location !== undefined) {
      ...
      return cached; // Return the full cached object (or string for legacy)
    } else {
...
async function addFlagToUsername(usernameElement, screenName) {
  ...
  try {
    ...
    // Get location data (may be string for legacy or object { location, locationAccurate })
    const locationData = await getUserLocation(screenName);
    // Handle both legacy string format and new object format
